...
func RunWorker(ctx context.Context, db *database.DB, getZoomToken ZoomTokenFunc, getTeamsToken TeamsTokenFunc, getGoogleMeetToken GoogleMeetTokenFunc, store storage.Interface, storagePrefix string, pollInterval, lockDuration time.Duration, jobProcessor *utils.JobProcessor, onJobReady OnJobReadyFunc) {
	if pollInterval <= 0 {
	...
	for {
		...
		case <-ticker.C:
			...
			runOne(ctx, db, job, getZoomToken, getTeamsToken, getGoogleMeetToken, store, storagePrefix, jobProcessor, onJobReady)
		}
	}
}

func runOne(ctx context.Context, db *database.DB, job *models.SessionProcessingJob, getZoomToken ZoomTokenFunc, getTeamsToken TeamsTokenFunc, getGoogleMeetToken GoogleMeetTokenFunc, store storage.Interface, storagePrefix string, jobProcessor *utils.JobProcessor, onJobReady OnJobReadyFunc) {
	defer func() {
		// Ensure unlock on panic
		_ = db.UnlockSessionProcessingJob(ctx, job.ID)
	}()
	_ = RunJob(ctx, db, job, getZoomToken, getTeamsToken, getGoogleMeetToken, store, storagePrefix, jobProcessor, onJobReady)
}
