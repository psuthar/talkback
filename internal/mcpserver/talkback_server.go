...
func NewTalkbackMCPServer(cfg TalkbackServerConfig) *mcp.Server {
	...
	opts := &mcp.ServerOptions{
		Instructions: "TalkBack MCP: tools include health_check; with DATABASE_URL, get_session_metadata, list_sessions (same ACL as GET /api/sessions), get_session_decisions, get_decisions_by_topic (match premise/primary_decision/decision_outcome by topic across accessible sessions), get_session_action_items (ephemeral LLM action items on read), " +
