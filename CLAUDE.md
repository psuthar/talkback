...
9. If FULL_AUTO was requested, follow `docs/agent/workflow-full-auto.md`. Two invocation keywords exist:
   - **`implement SCRUM-XX FULL_AUTO`** (default) — polling path. Agent polls the gate + `mergeable_state` every 30s on a 40-min budget. On PASS+clean, the post-merge close-out (merge + local cleanup + Jira → Done + closure comment) is delegated to **`python -m scripts.full_auto.close <TICKET> --pr <N> --path polling`** (Epic SCRUM-529; see `docs/agent/full-auto-scripts.md`). On user-override squash-merge, pass `--path manual-override` instead. Worktree-based runs continue to use the prose path in `workflow-full-auto.md` (close.py doesn't yet handle ExitWorktree + SCRUM-388 primary-tree FF). Closure comment names `"polling path (default)"` as the path indicator. Post-SCRUM-529 behavior: dirty `ops/define-kpis/lint-runs.log` is auto-stashed across the main checkout (SCRUM-534), `actions_taken` ends with a grep-able `close.py succeeded:` / `close.py aborted:` / `close.py dry-run:` summary line (SCRUM-536), and the JSON output exposes a tri-state `state_file_status` field replacing the older `state_file_updated` bool (SCRUM-538). **No claude.ai routine quota consumed.**
