...
9. If FULL_AUTO was requested, follow `docs/agent/workflow-full-auto.md`. Two invocation keywords exist:
   - **`implement SCRUM-XX FULL_AUTO`** (default) — polling path. The full workflow is delegated to four sibling Python scripts (Epic SCRUM-541 extended SCRUM-529): **`start.py`** (fetch ticket + lint + In Progress + branch), **`review.py`** (open PR + lint body + completion comment + In Review), **`poll.py`** (silent gate + `mergeable_state` polling until terminal), and **`close.py`** (merge + local cleanup + Jira → Done + closure comment). Agent invokes each in turn — see `docs/agent/full-auto-scripts.md` for the per-script invocation contract. On PASS+clean, `close.py` runs with `--path polling`; on user-override squash-merge, pass `--path manual-override` instead. Worktree-based runs continue to use the prose path in `workflow-full-auto.md` (close.py doesn't yet handle ExitWorktree + SCRUM-388 primary-tree FF). All four scripts share a consistent contract: structured JSON output, `actions_taken` ending with a grep-able `<script>.py succeeded:` / `aborted:` / `dry-run:` summary line (SCRUM-536), and enum-string fields (e.g. `state_file_status` SCRUM-538, `lint_status` SCRUM-542, `terminal_state` SCRUM-544) replacing earlier ambiguous booleans. **No claude.ai routine quota consumed.**
