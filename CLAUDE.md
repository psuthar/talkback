...
- **Env vars** (all are process-level env vars; `./scripts/setup-mcp-config.sh` copies `DATABASE_URL`, `TALKBACK_MCP_ACTING_USER_ID`, and `TALKBACK_MCP_KEY_USER_MAP_JSON` into `.cursor/mcp.json` when set in the shell you run it from):
  - `TALKBACK_MCP_URL` — **remote mode only (SCRUM-85):** set to the deployed server URL (e.g. `https://talkback-mcp.onrender.com`) before running `setup-mcp-config.sh`. When set, the script generates a `url`-based config entry (StreamableHTTP) instead of `command`/`args`; no local Go process is needed. The `Authorization: Bearer <TALKBACK_MCP_API_KEY>` header is included automatically. Unset = local stdio mode (default, unchanged).
