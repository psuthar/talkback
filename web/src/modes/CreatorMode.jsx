...
export function CreatorMode({
  ...
  return (
    <>
      ...
      {/* SCRUM-476: session-level processing banner (Fetch→Download→Parse→
          Chunk→Embed→Ready stage chain + "Preparing playback…" copy)
          removed. The SCRUM-468 inline placeholder row in
          MaterialsTreePanel's VIDEOS list is the canonical in-progress
          indicator now; the duplicated top banner added vertical noise
          without unique signal. Retry / reconnect affordances move with
          the placeholder row. The legacy zoom-source ingestion banner
          (was: a second top-of-page banner when ingestionStatus.source
          === 'zoom') is also removed for the same reason. */}
      {/* Three-panel grid */}
