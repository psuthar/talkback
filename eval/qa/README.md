...
| Path | Purpose |
|------|---------|
| `fixture_fact_inventory.json` | Full candidate case list (25 cases) with source refs and expected keywords/status. |
| `eval_cases_v1.json` | 25-case harness set with hallucination constraints (aligned with FF-001–FF-025). |
| `expected_scores_v1.json` | Per-case correctness/hallucination targets and weights (FF-001–FF-025). |
...
- **`eval_cases_v1`** now explicitly covers **FF-001..FF-025**; keep it aligned with both `fixture_fact_inventory.json` and `expected_scores_v1.json`.
