...
| 3 | `internal/utils/polish.go` (decision-extraction polish pass) | Worker / pipeline | LLM output or chunks (session-content); polish prompt (system) | Structured (decisions) or refined text | Injection-via-transcript; malformed JSON output | 4c (schema validation) |
| 4 | `internal/obsworker/llm.go` (observability worker) | Cron / internal | Log/metric content (system); analysis prompt (system) | Analysis text | Low — all inputs are system-controlled | (write-path only — Slice 5) |
