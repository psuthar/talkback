...
| `DATABASE_URL` | No | When set at process start, the server registers **`get_session_metadata`**, **`get_session_decisions`** (persisted premise / primary_decision / decision_outcome / decision_stances — SCRUM-55), **`search_session`**, **`search_session_content`** (same behavior as `search_session`), **`get_session_raw_chunks`**, **`get_session_retrieval_context`** (same behavior as `get_session_raw_chunks`), **`get_session_source_chunks`**, **`ask_session`**, and **`ask_session_question`** (same behavior as `ask_session`) (Postgres). When unset, only `health_check` is available. |
...
| `get_session_metadata` / `get_session_decisions` / `search_session` / `search_session_content` / `get_session_raw_chunks` / `get_session_retrieval_context` / `get_session_source_chunks` / `ask_session` / `ask_session_question` | Above plus `DATABASE_URL` and `TALKBACK_MCP_ACTING_USER_ID` (must be a valid user UUID in your DB). **`get_session_decisions`** needs only DB (no OpenAI). **`search_session`** (or **`search_session_content`**), **`get_session_raw_chunks`** (or **`get_session_retrieval_context`**), **`get_session_source_chunks`** (when indexing), and **`ask_session`** (or **`ask_session_question`**) need **`OPENAI_API_KEY`** (embeddings; Q&A also uses the LLM) — same embedding stack as web session Q&A. |
...
When **`DATABASE_URL`** is set at process start, the server opens Postgres through [`internal/database`](../internal/database) and registers **`get_session_metadata`** and **`get_session_decisions`**. The acting user is the TalkBack user UUID from **`TALKBACK_MCP_ACTING_USER_ID`** (wired into the request context after API-key middleware on `tools/call`). Access is allowed for **global admins** or users who pass **`UserCanAccessSession`** for that session — same rules as the web app. These tools do **not** return transcript or material bodies.
