...
| `TALKBACK_MCP_MAX_CROSS_SESSION_CHUNKS` | No | **SCRUM-74:** Max **chunk+embedding** rows loaded into memory for one **`search_all_sessions`** call (ranking uses this working set). Default **50 000** when unset. Values below **1000** are treated as invalid and fall back to the default; values above **500 000** clamp to **500 000**. Prevents unbounded RAM use when many sessions have large indexes. |
