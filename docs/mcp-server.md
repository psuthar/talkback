...
| `get_session_decisions` | Input: `session_id` (UUID). Output: **`schema_version`** (`1`), **`premise`**, **`primary_decision`**, **`decision_outcome`** (nullable strings from DB), **`decision_stances`** (array of persisted stances: `id`, `session_id`, `user_id`, `stance`, optional `rationale`, `user_email`, `created_at`, `updated_at`). **v1: persisted fields only** — no LLM extraction (SCRUM-55). Same session ACL as `get_session_metadata`. Implemented in `internal/mcpserver/session_decisions.go`. |
