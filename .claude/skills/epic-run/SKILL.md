...
Execute all child tickets of a Jira Epic in one automation loop using FULL_AUTO behavior per child, while enforcing strict epic sequencing and halt semantics.
...
- `run epic SCRUM-XX`: start fresh (fails if an active state file exists)
- `continue epic ...`: resume and drain remaining non-Done children unless HALT recurs

## Runtime Contract

For each child ticket during epic execution:

1. Execute `implement <KEY> FULL_AUTO`.
2. Respect epic ordering and merge-gate requirements from policy docs.
3. On HALT conditions, stop the full epic immediately and wait for explicit user resume.

## Start Algorithm

1. Check `.epic-run/<EPIC>.json`.
   - If present and not complete, require `continue epic`.
2. Query Jira children ordered by creation:
   - `parent = <EPIC> AND statusCategory != Done ORDER BY created ASC`
3. Build work list using parallel marker convention from policy (`parallel-ok` label or `Parallel: yes` description line).
4. Write initial state file.
5. Transition epic to In Progress (idempotent).
6. Execute work list in order.

## Execution Loop

### Sequential item

1. Transition child to In Progress before implementation work.
2. Create/checkout `feat/<ticket-key>` from current `main`.
3. Run `implement <ticket-key> FULL_AUTO`.
4. If pass: record result, ensure child Jira Done, continue.
5. If not pass: HALT.

### Parallel batch

1. Transition each batch child to In Progress.
2. Start per-ticket `implement <ticket-key> FULL_AUTO` concurrently.
3. Wait for all outcomes.
4. If all pass: record all and continue.
5. If any fail/halt: HALT entire epic.

## Resume Algorithm (`continue epic`)

1. Sync local git first:
   - `git fetch origin`
   - `git checkout main`
   - `git pull --ff-only origin main`
2. Read state file if present.
3. Ensure epic is In Progress (idempotent).
4. Re-query Jira non-Done children as source of truth.
5. For each child in order:
   - Done already: skip
   - PR already merged but Jira not Done: reconcile close-out only (no re-merge)
   - PR open and gates unresolved: HALT
   - PR open and gates now pass: resume FULL_AUTO polling path
   - No implementation yet: run `implement <KEY> FULL_AUTO`

## Manual Merge Reconciliation

If user already squash-merged:

1. Do not call merge again.
2. Transition Jira child to Done.
3. Cleanup local branch for that ticket:
   - sync `main`, delete `feat/<ticket-key>` if present
4. Mark ticket done in state file with `final_gate: "manual_override"`.
5. Continue epic processing.
...
1. Persist state:
   - `status: halted`
   - `awaiting_human: true`
   - `halt_reason`, `halted_at`
2. Post Jira comment on epic with:
   - completed tickets
   - halted ticket + reason
   - remaining tickets
   - resume instruction (`continue epic <EPIC>`)
3. Stop; do not self-resume.

## Finish

When no non-Done children remain:

1. Mark state `complete`.
2. Transition epic to Done.
3. Post epic completion summary comment with merged tickets.
...
- Do not skip tickets silently.
- Do not start ticket N+1 before ticket N is merged and Done unless explicit parallel markers permit batching.
- Do not self-resume after halt without explicit user instruction.
- Treat Jira `statusCategory != Done` as source of truth for remaining work.
