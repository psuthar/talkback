...
> **Synthetic recordings.** The candidate, transcripts, scorecards, Slack threads, and audio in `videos/*.mp4` are all fictional. Audio is generated by OpenAI TTS via `tools/render_videos.py`; the MP4 and VTT artifacts are not committed to git (regenerate locally). When demoing, disclose to the audience that the recordings are synthetic.
